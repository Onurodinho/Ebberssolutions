...
if [[ -z "${CLOUDFLARE_API_TOKEN:-}" || -z "${CLOUDFLARE_ACCOUNT_ID:-}" ]]; then
  ...
  echo ""
  echo "── STAP 3 (handmatig in dashboard) ──"
  echo "  1. https://dash.cloudflare.com → Workers & Pages → Worker 'ebbers'"
  echo "     (Geen project? Maak eerst deploy: zie STAP 4 hieronder)"
  echo "  2. Settings → Domains & Routes → Add → Custom Domain"
  echo "     → ebberssolutions.com"
  echo "     → www.ebberssolutions.com"
  echo "  3. DNS → Records voor ebberssolutions.com:"
  echo "     → Verwijder ALLE oude records die naar Netlify wijzen"
  echo "       (CNAME naar *.netlify.app, Load Balancer, Workers-route naar Netlify)"
  echo "     → Custom Domain maakt automatisch nieuwe records aan"
...
echo ""
echo "Deploy klaar. Controleer headers:"
sleep 3
if curl -sI https://ebberssolutions.com/ 2>/dev/null | grep -qi 'netlify'; then
