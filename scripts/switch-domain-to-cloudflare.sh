...
if [[ -z "${CLOUDFLARE_API_TOKEN:-}" || -z "${CLOUDFLARE_ACCOUNT_ID:-}" ]]; then
  ...
  echo "     → Alleen ebberssolutions.com als Custom Domain (NIET www via Worker)"
  echo "     → Verwijder www.ebberssolutions.com als die in de lijst staat (breekt deploy)"
  echo "  2b. Rules → Redirect Rule: www.ebberssolutions.com → https://ebberssolutions.com (301)"
  echo "  2c. Builds → Deploy command: npm run deploy:ci (niet wrangler deploy)"
